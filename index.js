...
const totalContributions = GAMES_JSON.reduce((total, game) => {  return  total + game.backers;
}, 0);
...
function filterUnfundedOnly() {
  ...
  const listGames  = GAMES_JSON.filter(game => game.goal > game.pledged);
  console.log("unfunded" + listGames.length);
  // use the function we previously created to add the unfunded games to the DOM
  addGamesToPage(listGames);
}
...
function filterFundedOnly() {
  ...
  const listGames  = GAMES_JSON.filter(game => game.pledged >= game.goal);
  console.log("funded" + listGames.length);
...
const displayStr = `A total of $${totalAmountRaised.toLocaleString()} has been rasied for ${GAMES_JSON.length} games. Currently, ${numUnfunded} game${numUnfunded === 1 ? "" : "s"} remain unfunded. We need your help to fund these amzaing games!`;
...
topGameElement.textContent = `${firstGame.name} - $${firstGame.pledged.toLocaleString()}`;
